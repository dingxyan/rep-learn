...
(function() {
	...
	function addCube(localMesh,key){
		//创建cube
		localMesh.geometry.computeBoundingBox();
		var boundingBox = localMesh.geometry.boundingBox.clone();
		console.log(111, boundingBox)
		//boundingBox
		var scale = localMesh.scale.x;
		var x = (boundingBox.max.x - boundingBox.min.x)*scale;
		var y = (boundingBox.max.z - boundingBox.min.z)*scale;
		var z = (boundingBox.max.y - boundingBox.min.y)*scale;
		console.log(222,x,y,z)
		// var cubeGeometyr = new THREE.BoxBufferGeometry( 3.838908,4.0287779, 1.810306 );
		var cubeGeometyr = new THREE.BoxBufferGeometry(x,y,z);
		var cubeMaterial = new THREE.MeshBasicMaterial( {color: 0x00ff00,transparent:true,opacity:0.5} );
		var cube = new THREE.Mesh( cubeGeometyr, cubeMaterial );
		//position
		var a = localMesh.position.x;
		var b;
		if(key && key==3){
			b = localMesh.position.y+scale/2;
		}else{
			b = localMesh.position.y+scale;
		}
		
		var c = localMesh.position.z;
		console.log(333,a,b,c)
		// cubeA.position.set( 0,2,-10 );
